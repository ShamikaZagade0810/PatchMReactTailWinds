...
const ViewClientWiseSyncPolicy = () => {
    ...
    const initialApiReq = async () => {
        try {
            setLoading(true);
            const res = await getAllViewClientWiseSyncPolicy();
            console.log("API Response:", res);
            // adjust based on backend response structure
            setclientsyncPolicy(res?.data?.data || res?.data || []);
        } catch (error) {
    ...
    const ipaddress = [
        ...
        { value: "192.168.1.20", label: "192.168.1.20" },
        { value: "192.168.1.21", label: "192.168.1.21" }
    ];
    ...
    const handleEdit = (item, index) => {
        ...
        console.log("time:", item.scheduleTime, typeof item.scheduleTime);

        setEditData({
            ...item,
             time: item.scheduleTime ? item.scheduleTime.substring(0, 5) : "",
            ipAddress: item.ipAddress?.[0]?.split(",")?.map((ip) => ({
                value: ip.trim(), label: ip.trim()
            })) || []
        });
    ...
    try {
        ...
        const inputData = {
            ...
            port: editData?.port,
            patchUpdateParameter: String(editData?.parameter),
        ...
        const response = await updateViewClientWiseSyncPolicy(inputData);
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            const response =  await deleteViewClientWiseSyncPolicy(inputData , deleteId);;
                        ...
                        {loading ? (
                            ...
                            clientsyncPolicy.map((item, index) => (
                                <tr key={index} className="border-b border-white/10 hover:bg-[#172033] transition" >
                                    ...
                                    <td className="p-3 break-words">
                                        {{
                                            2: "Notify for download",
                                            3: "Notify for download and notify for install",
